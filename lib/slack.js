...
async function postVisitorPing({ companyName, page, city, region }) {
  const location = [city, region].filter(Boolean).join(', ');
  const text = `🌐 Website visit: ${companyName || 'Unknown company'} viewed ${page || 'your site'}`;
    ...
    [
      {
        ...
        text: {
          type: 'mrkdwn',
          text: `*🌐 Website visitor* — *${companyName || 'Unknown company'}*${page ? ` viewed \`${page}\`` : ''}${location ? `\n_${location}_` : ''}`,
        },
      },
    ],
    text
...
module.exports = { postMessage, postSignalPing, postDigest, postVisitorPing };stDigest };
